...
export default function LegacyStudiosImport() {
  ...
  const invalid = studios.filter((s) => !s.contactEmail);
    ...
    <div className="space-y-6">
      ...
      <section className="card space-y-3">
        ...
        <p className="text-xs text-[color:var(--fg-2)]">
          ...
          <code className="font-mono text-[11px]">studio, slug, contact_email, emails (;-sep), hourly_rate, active, book_title, isbn, narrator, net_hours, notes</code>
        </p>
        ...
        <textarea
          ...
          placeholder={'studio,contact_email,hourly_rate,book_title,narrator,net_hours\nO2 Studio,studio@example.com,25,The Example,Narrator,6.5'}
        />
        {unmapped.length > 0 && (
        ...
        <section className="card space-y-3">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {studios.slice(0, 40).map((s, i) => {
              ...
              return (
                <div key={i} className={`rounded-xl border px-3 py-2.5 ${s.contactEmail ? 'border-slate-100' : 'border-red-200 bg-red-50'}`}>
                  <p className="text-sm font-bold text-[color:var(--samawy-ink)]">{s.name}</p>
                  <p className="text-xs text-[color:var(--fg-2)] truncate">{s.contactEmail || (isArabic ? '⚠ بدون بريد' : '⚠ no email')}{s.emails?.length ? ` +${s.emails.length}` : ''}</p>
                  <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1.5 text-xs text-[color:var(--fg-2)]">
                    <span className="flex items-center gap-1"><Users className="h-3 w-3" />{1 + (s.emails?.length ?? 0)}</span>
